 ...
 *
 * This is the *populated* demo batch. Its sibling, the 9999 fixture in
 * scripts/demo, is a correctness probe: thirty-odd rows, each one hand-written
 * to prove a specific rule. This one exists for the opposite reason — to answer
 * "what does the app look like with a real season's worth of data in it", which
 * is a question no fixture of thirty rows can answer. Empty states, five-row
 * tables and single-bar histograms tell you nothing about whether a page holds
 * up, and neither does an analytics query that never sees a hundred rows.
 *
 ...
 *     than on the two hand-picked cases the 9999 fixture covers.
