...
describe("authenticateWithPesu", () => {
  it("accepts a successful login whose profile omits every optional field", async () => {
    globalThis.fetch = mockResponse({
      ...
      profile: { srn: "PES2UG24CS036" },
    }) as never;

    const result = await authenticateWithPesu("PES2UG24CS036", "irrelevant");
    expect(result.ok).toBe(true);
    if (result.ok) {
      expect(result.profile.srn).toBe("PES2UG24CS036");
      // With no name upstream, the SRN stands in rather than the login failing.
      expect(result.profile.name).toBe("PES2UG24CS036");
    }
  });
  ...
  it("falls back to the typed SRN when the profile omits it", async () => {
    ...
    const result = await authenticateWithPesu("pes2ug24cs036", "irrelevant");
    expect(result.ok).toBe(true);
    if (result.ok) expect(result.profile.srn).toBe("PES2UG24CS036");
  });
  ...
  it("reports a wrong password as a wrong password", async () => {
    ...
    const result = await authenticateWithPesu("PES2UG24CS036", "wrong");
    expect(result.ok).toBe(false);
    ...
    globalThis.fetch = vi.fn(async () => {
      ...
      return new Response(
        JSON.stringify({
          ...
          message: "Login successful.",
          timestamp: "x",
          profile: { srn: "PES2UG24CS036", name: "A Student" },
        }),
  ...
  it("never puts the password in the returned message", async () => {
    ...
    const result = await authenticateWithPesu("PES2UG24CS036", "a-secret-value");
    expect(result.ok).toBe(false);
